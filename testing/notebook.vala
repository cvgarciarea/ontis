...
public class Notebook: Gtk.Box {
    ...
    public void append_page(string tab_label, Gtk.Widget child) {
...
void new_tab_cb(Notebook notebook) {
    a ++;
    notebook.append_page("Tab" + a.to_string(), new Gtk.Button.with_label("Widget" + a.to_string()));
    notebook.set_current_page(notebook.n_pages - 1);
}

void minimize(Notebook notebook) {
    win.iconify();
}

void turn_maximize(Notebook notebook) {
    if (win.is_maximized) {
        win.unmaximize();
    } else {
        win.maximize();
...
void main(string[] args) {
    ...
    notebook.set_show_buttons(true);
    notebook.remove(notebook.switcher);
    notebook.remove(notebook.box);
    notebook.new_tab.connect(new_tab_cb);
    notebook.minimize.connect(minimize);
    notebook.turn_maxmizie.connect(turn_maximize);
    notebook.close.connect(_close);
    ...
    win.set_titlebar(notebook.switcher);

    //box.pack_start(notebook.switcher, false, false, 0);
    box.pack_start(notebook.box, true, true, 0);
